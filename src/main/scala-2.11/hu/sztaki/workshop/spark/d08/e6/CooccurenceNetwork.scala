...
object CooccurenceNetwork {
  ...
  def main(args: Array[String]) {
    ...
    // Is the distribution tail long? Check the topics that have only 1-10 occurrences
    val valueDist = topicCounts.groupBy(_._2).mapValues(_.size)
    valueDist.toSeq.sorted.take(10).foreach(println)

    // Get co-occurrence pairs (without GraphX)
    ...
    // Construct a co-occurrence network with GraphX
    ...
    // Get the top 10 largest connected components
    ...
    // Compute average path length with Pregel
